...
# This funtion uses sfm.triangulatePoints2() function.
# If sfm is not available, this function will not work.
# The sfm is a module in opencv contrib that provides functions for structure from motion (SfM) tasks.
# The sfm module is not available in the standard opencv package (as of June 2025). 
#
# Function multi_camera_triangulation2: given camera parameters of multiple cameras (i.e., rvecs, 
...
#       triangulated_points, projected_pointss = multi_camera_triangulation2(rvecs, tvecs, 
#           cmats, dvecs, img_pointss)
def multi_camera_triangulation2(rvecs, tvecs, cmats, dvecs, img_pointss):
    # number of cameras
    n_cameras = len(rvecs)
    if n_cameras < 2:
        print("# multi_camera_triangulation2(): Error: the number of cameras is less than 2.")
        return None
    if n_cameras != len(tvecs) or n_cameras != len(cmats) or n_cameras != len(dvecs) or n_cameras != len(img_pointss):
        print("# multi_camera_triangulation2(): Error: the number of cameras is not consistent.")
        print("# multi_camera_triangulation2(): len(rvecs) is %d." % len(rvecs))
        print("# multi_camera_triangulation2(): len(tvecs) is %d." % len(tvecs))
        print("# multi_camera_triangulation2(): len(cmats) is %d." % len(cmats))
        print("# multi_camera_triangulation2(): len(dvecs) is %d." % len(dvecs))
        print("# multi_camera_triangulation2(): They should be the same. Otherwise the function cannot work.")
        return None
    # number of points
    n_points = len(img_pointss[0])
    if n_points != len(img_pointss[1]):
        print("# multi_camera_triangulation2(): Error: the number of points is not consistent.")
        for i in range(n_cameras):
            print("# multi_camera_triangulation2(): len(img_pointss[%d]) is %d." % (i, len(img_pointss[i])))
        print("# multi_camera_triangulation2(): They should be the same. Otherwise the function cannot work.")
        return None
    # 
    # convert img_pointss to undist_pointss
    # img_pointss[i] is a n_points x 2 numpy array, where each row is a 2D point in the i-th camera
    # undist_pointss[i] is a n_points x 2 numpy array, where each row is a 2D undistorted point in the i-th camera 
    undist_pointss = []
    for i in range(n_cameras):
        # undistort the image points using cv2.undistortPoints()
        undist_points = cv2.undistortPoints(img_pointss[i].reshape(-1, 1, 2), cmats[i], dvecs[i]).reshape(-1, 2).T
        undist_pointss.append(undist_points)
    # calculate projection matrices for each camera based on cmats, rvecs, and tvecs
    # projection_matrices: a list of n_cameras elements, where each element is a 3x4 numpy array
    projection_matrices = []
    for i in range(n_cameras):
        rmat, _ = cv2.Rodrigues(rvecs[i])  # convert rotation vector to rotation matrix
        proj_matrix = np.hstack((rmat, tvecs[i].reshape(3,1)))
        projection_matrices.append(proj_matrix)
    # triangulate points using cv2.sfm.triangulatePoints2()
    # triangulated_points: a n_points x 3 numpy array, where each row is a 3D point
    triangulated_points = (cv2.sfm.triangulatePoints(undist_pointss, projection_matrices)).T
...
# a unit test of multi_camera_triangulation2 (which is based on sfm.triangulatePoints2())
# This function does:
...
#     cameras by calling multi_camera_triangulation2. Store the results in triangulated_pos3ds, 
...
#    from multi_camera_triangulation import multi_camera_triangulation
    triangulated_pos3ds = multi_camera_triangulation2(rvecs, tvecs, cmats, dvecs, img_points)
...
def unit_test_gui_multi_camera_triangulation2_v01():
    # create a tk window, with a given window title
    import tkinter as tk
    from tkinter import ttk
    window = tk.Tk()
    window.title("Unit Test of multi_camera_triangulation2 (sfm based). vince")
    ...
    # a button titled with "multi_camera_triangulation2"
    # This button calculates the 3D points from the 2D points in multiple cameras
    # by calling multi_camera_triangulation2. The 3D points are printed to the text output.
    ...
    # a button 
    def bt_event_multi_camera_triangulation2():
        ...
        triangulated_points, projected_pointss = multi_camera_triangulation2(rvecs, tvecs, cmats, dvecs, img_pointss)
        ...
        # return
    button_multi_camera_triangulation2 = ttk.Button(window, text="multi_camera_triangulation2 with projection noise (in pixels):", 
                                         command=bt_event_multi_camera_triangulation2)
    button_multi_camera_triangulation2.grid(row=irow, column=0)
...
if __name__ == '__main__':
    unit_test_gui_multi_camera_triangulation2_v01()
