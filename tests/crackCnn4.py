...
def inputdlg(prompt: list, name: str='Input Dialog', numlines: int =1, 
             defaultanswer:list=[]):
    """
    ...
    name : str, optional
        Name of the dialog window. The default is 'Input Dialog'.
    numlines : int, optional
        number of lines per answer. The default is 1.
    defaultanswer : list of default answers, optional
...
def trainImgFilesToImgArray(files: list, imread_grayscale=False):
    """
...
traindataFile = r'D:\yuansen\ImPro\improMeasure\examples\crackTraining\train_patch_%d_%d_gray_xy.npz' % (patchSize, patchSize)
trainedModelFile = r'D:\yuansen\ImPro\improMeasure\examples\crackTraining\model_patch_%d_%d_gray.h5' % (patchSize, patchSize)

imgArray = trainImgFilesToImgArray(imgFiles, imread_grayscale=True)
mskArray = trainImgFilesToImgArray(mskFiles, imread_grayscale=True)
slicedImgs = imgGridSlice(imgArray, patchSize, patchSize)
slicedMsks = imgGridSlice(mskArray, patchSize, patchSize)
categ = slicedMasksToCategorical(slicedMsks)
#showImgMskCateg(slicedImgs[0:196,:,:,:], slicedMsks[0:196,:,:,:], categ, (14, 14))
...
if useBalanced:
    ...
    validRatio = 0.1
    trainCount = int(totalCount * (1 - validRatio) + 0.5)
...
if runDataAug:
    ...
    datagen.fit(x_train)
    my_logs = my_model.fit(
                datagen.flow(x_train, y_train, batch_size = my_batch_size), \
                validation_data = datagen.flow(x_valid, y_valid, batch_size=my_batch_size),\
                epochs = my_epochs, verbose = 1, 
                steps_per_epoch=x_train.shape[0] // my_batch_size)
else:
    my_logs = my_model.fit(x_train, y_train, batch_size = my_batch_size, \
              epochs = my_epochs, verbose = 1, \
              validation_data= (x_valid, y_valid))
fitToc = time.time()
print("Time for fitting (training): %.2f sec." % (fitToc - fitTic))

my_model.save(r'D:\yuansen\ImPro\improMeasure\examples\crackTraining\model_patch_32_32_gray_c323232_d32_dataAug1.h5')
