...
traindataFile = r'D:\yuansen\ImPro\improMeasure\examples\crackTraining\train_patch_64_64_xy.npz'
imgArray = trainImgFilesToImgArray(imgFiles)
mskArray = trainImgFilesToImgArray(mskFiles)
slicedImgs = imgGridSlice(imgArray, 64, 64)
slicedMsks = imgGridSlice(mskArray, 64, 64)
...
x1 = layers.Conv2D(filters=32, kernel_size=3, activation='relu')(ipt)
x2 = layers.Conv2D(filters=16, kernel_size=3, activation='relu')(x1)
x3 = layers.Conv2D(filters=16, kernel_size=3, activation='relu')(x2)
x4 = layers.GlobalAveragePooling2D()(x3)
x5 = layers.Dense(16, activation='relu')(x4)
c10 = layers.Dense(num_classes, activation='softmax')(x5)
my_model = models.Model(inputs=ipt, outputs=c10)
my_model.compile(loss=losses.categorical_crossentropy, \
              optimizer = optimizers.SGD(),metrics = ['accuracy'])
my_epochs = 100
tic = time.time()
my_logs = my_model.fit(x_train, y_train, batch_size = 128, \
