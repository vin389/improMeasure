...
This module defines the `Camera` class and several utility functions for working with both extrinsic and intrinsic
camera parameters in computer vision and photogrammetry applications. It offers initialization, rotation,
file I/O, undistortion, and visualization capabilities. The camera model is based on OpenCV's pinhole model.

=========================
CAMERA PARAMETERS OVERVIEW
=========================
- **Extrinsic parameters** describe the camera's position and orientation in the world:
    - `rvec`: 3x1 rotation vector in Rodrigues format (from world to camera coordinate system)
    - `tvec`: 3x1 translation vector (camera origin in world coordinates)

- **Intrinsic parameters** describe the internal characteristics of the camera:
    - `cmat`: 3x3 camera matrix: [[fx, 0, cx], [0, fy, cy], [0, 0, 1]]
    - `dvec`: 1×n distortion coefficients (n=4~14): ordering is [k1, k2, p1, p2, k3, k4, k5, k6, s1, s2, s3, s4, taux, tauy].
              Not all coefficients are always used; only the relevant number depending on distortion model.
...
### Constructor
- `__init__(imgSize=None, rvec=None, tvec=None, cmat=None, dvec=None)`:
  Initializes a camera. Defaults are identity for `cmat`, zeros for `rvec`, `tvec`, and 5-zero distortion.

### Initialization Functions
- `initByKeyin(keyin='')`:
  Initializes camera parameters from user text input (e.g., from a script or manual input). Expects 21–31 numbers.

- `initByNumpyFile(filepath='')`:
  Loads parameters from a file (text or `.npy`). Uses format [imgSize, rvec, tvec, cmat, dvec].

- `initByAsk()`:
  Interactive initialization. Prompts user to choose between key-in or loading from a file.

### Intrinsic Accessors
- `fx(), fy()` – Focal lengths in pixels
- `cx(), cy()` – Principal point coordinates in pixels
- `cx_ratio(), cy_ratio()` – Normalized principal point positions (0.5 = center)
- `k1(), k2(), k3(), k4(), k5(), k6()` – Radial distortion coefficients
- `p1(), p2()` – Tangential distortion coefficients

### Field of View Estimators
- `fovx(), fovy()`:
  Returns horizontal and vertical field-of-view angles in degrees, estimated from focal length and image size.

### Extrinsic Accessors
- `rmat()`:
  Returns the 3×3 rotation matrix from `rvec` using Rodrigues formula.

- `rmat44()`:
  Returns 4×4 homogeneous extrinsic matrix for transforming world coordinates to camera coordinates.

- `campos()`:
  Returns camera position in world coordinates (inverse of extrinsic matrix).

### Extrinsic Modifiers
- `setRvecTvecByPosAim(pos, aim)`:
  Computes `rvec` and `tvec` given the camera position and the point it aims at.

- `setRvecTvecByPosYawPitch(pos, yaw, pitch)`:
  Sets extrinsic matrix based on camera position and yaw/pitch (in degrees). Assumes Z-up and X-right coordinate system.

- `rotateCamera(rotAxis, coordSys, rotAngleInDeg)`:
  Rotates the camera around a given axis (in global or local frame), updating `rvec`/`tvec` without changing position.

### Intrinsic Modifiers
- `setCmatByImgsizeFovs(imgSize, fovs, pp=None)`:
  Sets `cmat` from image size and field-of-view (x and y). Optional principal point.

### Visualization
- `plotCameraPyramid(color='green', alpha=0.5, axes=[], pyramid_height=0.1)`:
  Plots a camera frustum in 3D axes using `matplotlib`, based on the current intrinsic and extrinsic parameters.

### File I/O
- `saveToFile(file)`:
  Saves all parameters into a readable file (text format). The file consists of a column vector containing:
  [width, height, rvec_x, rvec_y, rvec_z, tvec_x, tvec_y, tvec_z, 9 elements of cmat row-wise, and distortion coefficients].
  Lines beginning with `#` include human-readable descriptions and matrix formatting.

- `loadFromFile(file)`:
  Loads parameters from a file using the `readCamera()` utility. The file should match the format described above:
  a text file with one value per line and optional comments starting with `#`. The first two values are image width and height,
  followed by extrinsic (rvec, tvec), then intrinsic matrix, and distortion coefficients.

### Image Processing
- `undistort(img)`:
  Returns undistorted image using OpenCV’s `cv2.undistort()` with current `cmat` and `dvec`.

### Printing
- `__str__()`:
  Returns a human-readable multi-line string describing all camera parameters, including matrices and coefficients.
...
# >>> cam = Camera()
# >>> cam.setRvecTvecByPosAim([0, -100, 50], [100, 0, 100])
# >>> cam.setCmatByImgsizeFovs([1920, 1080], [90])
# >>> print(cam.fovx(), cam.fovy())
# >>> cam.saveToFile("cam1.txt")
# >>> cam2 = Camera()
# >>> cam2.loadFromFile("cam1.txt")
# >>> print(cam2.campos())
# >>> ax = cam2.plotCameraPyramid()

==================
DEVELOPMENT NOTES
==================
- All math follows OpenCV conventions for camera calibration.
- Matplotlib is used for plotting; OpenCV for camera and distortion functions.
- GUI support (Tkinter) and interactive input are provided for user-friendly configuration.
