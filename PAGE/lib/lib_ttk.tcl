...
namespace eval vTcl::widgets::ttk {
	...
	# Added by Rozen to mirror above
	proc update_position {target var} {
		global vTcl
        ## there is a trace on var to update the combobox
        ## first item in the list is the current index
        set sites [$target tabs]
	...
	proc config_position {target var} {
		global vTcl
		if {$var eq ""} {
			set style ""
		} else {
			set style "Tab_${var}.TNotebook"
		}
		$target configure -style $style

		# What follows is a workaround for a bug in the rendering of
		# TNotebook.
		# set x_option [place configure $target -x]
		# set x [lindex $x_option 4]
		# incr x
		# place configure $target -x $x
		# update
		# incr x -1
		# place configure $target -x $x
		event generate $target <Configure>
        update
		vTcl:active_widget $target
	}
	
	proc get_position {target} {
		global vTcl
		if {![info exists vTcl(w,opt,-_ttk_notebook_position)]} {
			set vTcl(w,opt,-_ttk_notebook_position) ""
			return ""
		} else {
			ttk::style configure TNotebook \
				 -tabposition  $vTcl(w,opt,-_ttk_notebook_position)
			return $vTcl(w,opt,-_ttk_notebook_position)
		}
	}
