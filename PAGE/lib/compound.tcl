...
namespace eval ::vTcl::compounds {
    ...
    proc createCompound {target type compoundName \
		...
		# If we are copying a menu, find the highest integer in a menu
		# widget name.
	    	set ll [list 0]
      ...
      "namespace eval \{::vTcl::compounds::${type}::[list $compoundName]\} \{\n"
        ...
        if {$class == "Toplevel"} {
           ...
           # append output "    set menu \$vTcl(copy_menu)x\n"
			set m [vTcl:new_widget_name Menu ""]
			#append output "    set menu .$m\n"
			set m_no [expr {$vTcl(menu_number) + 1}]
			append output "    set menu .mem$m_no\n"
	...
	if {$mainCmd != ""} {
        ...
        set vTcl(copy_width)    [winfo width $w_widget]
            # debugging statements. Add clipboard to terminal
            # dskip 6
            # dpr output
		    # dmsg final copy results

		    # Writes out the clipboard file so that it can be modified
		    # and read back in. A debugging tool. It requires that the
		    # PAGE invocation supply a project name, e.g. "page test".

            # debugging statements. Save copy.cpy file.
		    # set fn [file join "." copy.cpy]
		    # set chan [open $fn w]
		    # puts $chan $output
		    # close $chan
        return $output
...
#"
    ...
    proc autoPlaceCompound {type compoundName gmgr gopt} {
        ...
        if {$rootclass == "Toplevel"} {
			...
			regexp {^\.top\d+} $vTcl(w,insert) new_insert

		    set targ [vTcl:new_widget_name $namePrefix $::vTcl(w,insert)]
			
			# insertCompound $targ $type $compoundName $gmgr $gopt
			insertCompound $new_insert $type $compoundName $gmgr $gopt
			# insertCompound $vTcl(w,insert) $type $compoundName $gmgr $gopt
			vTcl:init_wtree
			return
        } else {
            set namePrefix cpd
...
proc insertCompound {target type compoundName {gmgr pack} {gopt ""}} {
	...
	# Debugging stuff: source copytest.cpy file.
	# May want to cp copy.cpy, copytest.cpy 

	# set fn [file join "." "copytest.cpy"]
	# set chan [open $fn r]
	# source $fn
	# dmsg After source
	
	if {$vTcl(copy_class) eq "Menu"} {
		regexp {^\.top\d+} $target target
	} elseif {$vTcl(copy_class) eq "Popupmenu"} {
		# Since the desired parent of the popup is a toplevel.
		regexp {^\.top\d+} $vTcl(w,insert)  top_part
		set menu_part ""
		# regexp {.pop\d+$}  $target menu_part
		# regexp {.m\d+$}  $target menu_part
		regexp {.pmu\d+$}  $target menu_part
	}
    ...
    # Debugging Statement KEEP.  Sends copy scratch file to terminal
    # dpr -s cmd
    # dmsg ################# End of cmd #################
