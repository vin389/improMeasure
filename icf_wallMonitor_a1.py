...
#                    cv.destroyWindow('tmplt')
                    ...
                    if istep <= 1:
                        ...
                        vel = ctrlPoints2d[istep - 1, icam, ipoint, :] \
                            - ctrlPoints2d[istep - 2, icam, ipoint, :]
                    srch_x = round(rect_x + vel[0] - 2 * rect_w)
                    srch_y = round(rect_y + vel[1] - 2 * rect_h)
                    srch_w = rect_w * 5
                    srch_h = rect_h * 5
                    if srch_x < 0:
                        srch_x = 0
...
#                    cv.waitKey(0)
        ...
        if istep == 0:
            ...
            ppx_default = min(ppx_ori_cam1_x, ppx_ori_cam1_z, ppx_ori_cam2_x, ppx_ori_cam2_z)
            ...
            if type(_ppx) == type(None):
                ...
                print("#   The default value (if you input 0 or -1) is %f" % ppx_default)
...
#                imgPts = nextPts.reshape(nOpfPtsY, nOpfPtsX, 2)
                ...
                field_uy_exp[1:-1,-1] = 2 * field_uy[:,-1] - field_uy[:,-2]
                field_uy_exp[ 0, 0] = 2 * field_uy[ 0, 0] - field_uy[ 1, 1] + 1e6
                field_uy_exp[ 0,-1] = 2 * field_uy[ 0,-1] - field_uy[ 1,-2] + 1e6
                field_uy_exp[-1, 0] = 2 * field_uy[-1, 0] - field_uy[-2, 1] + 1e6
                field_uy_exp[-1,-1] = 2 * field_uy[-1,-1] - field_uy[-2,-2] + 1e6
                ...
                if usePythonForStrainFields:
                    ...
                    for i in range(nOpfPtsY):
                        for j in range(nOpfPtsX):
                            ii = i + 1
                            jj = j + 1
                            field_exx[i,j] = (field_ux_exp[ii,jj+1] - field_ux_exp[ii,jj-1]) / (2 * dx)
                            field_eyy[i,j] = (field_uy_exp[ii-1,jj] - field_uy_exp[ii+1,jj]) / (2 * dy)
                            field_gxy[i,j] = (field_ux_exp[ii-1,jj] - field_ux_exp[ii+1,jj]) / (2 * dy)\
                ...
                if useNumbaForStrainFields == True:
                    ...
                    if usePythonForStrainFields == False:
                        field_exx, field_eyy, field_gxy = calcStrainFields(field_ux, field_uy, dx, dy)
                    else:
                        _field_exx, _field_eyy, _field_gxy = calcStrainFields(field_ux, field_uy, dx, dy)
                    toc = time.time()
                ...
                # write exx, eyy, and exy to files
                tic = time.time()
                ...
                toc = time.time()
                print("# It took %f sec. to write fields exx, exy, and gxy (engineering shear strain gamma) files  (Cam:%d, step %d)."
                      % (toc - tic, icam + 1, istep + 1))
                ...
                # vw = -1
                # cl = ' -30 30 '
                # sf = wdir_fields + '/Cam_%d_ux_cellSize_%d_cl30/Step_%d_Cam_%d_ux_cellSize_%d_cl30.JPG' % (icam+1, iCellSize, istep+1, icam+1, iCellSize)
                # viewField(bg,ff,pf,sf,cm,rm,co,cl)
                # cl = ' -10 10 '
                ...
                # viewField(bg,ff,pf,sf,cm,rm,co,cl)
                # cl = ' -1 1 '
                # sf = wdir_fields + '/Cam_%d_ux_cellSize_%d_cl01/Step_%d_Cam_%d_ux_cellSize_%d_cl01.JPG' % (icam+1, iCellSize, istep+1, icam+1, iCellSize)
                # viewField(bg,ff,pf,sf,cm,rm,co,cl)
                # # uy
                # bg = rectf_filename
                # ff = field_uy_file
                ...
                # cl = ' -1 1 '
                # sf = wdir_fields + '/Cam_%d_uy_cellSize_%d_cl01/Step_%d_Cam_%d_uy_cellSize_%d_cl01.JPG' % (icam+1, iCellSize, istep+1, icam+1, iCellSize)
                # viewField(bg,ff,pf,sf,cm,rm,co,cl,vw)
                # exx
                bg = rectf_filename
                ff = field_exx_file
...
def test1():
    icf_wallMonitor_a1(
        _files1=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall\leftPause\*.JPG',
        _files2=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall\rightPause\*.JPG',
        _cam1=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall\calib\Specimen03_Sony_L_20220321_101202_camera.txt',
        _cam2=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall\calib\Specimen03_Sony_R_20220321_101202_camera.txt',
        _ctrlp1file=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall\Sony_L_ctrlp.txt',
        _ctrlp2file=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall\Sony_R_ctrlp.txt',
        _ppx='2', 
        _xzzx='xz',
        _rexp='0.1',
        _wdir=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall'
        )
...
def test3():
    icf_wallMonitor_a1(
        _files1=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall_cracktest\L\*.JPG',
        _files2=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall_cracktest\R\*.JPG',
        _cam1=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall_cracktest\Specimen01_Canon_L_20220310_102517_camera_parameters.csv',
        _cam2=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall_cracktest\Specimen01_Canon_R_20220310_102517_camera_parameters.csv',
        _ctrlp1file=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall_cracktest\Canon_L_ctrlp.txt',
        _ctrlp2file=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall_cracktest\Canon_R_ctrlp.txt',
        _ppx='2.0', 
        _xzzx='xz',
        _rexp='0.1',
        _wdir=r'D:\yuansen\ImPro\improMeasure\examples\2022rcwall_cracktest'
        )
