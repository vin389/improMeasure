...
#    camParams = np.zeros(14, dtype=np.float64)
    ...
    btCoord3d = tk.Button(win, height=1, text='3D coordinates (World coord.)')
    btCoord3d.grid(row=irow, column=0, pady=0, ipady=0)
    irow += 1
    # edit text
    widgetHeight = 4
    edCoord3d = tk.Text(win, width=40, height=widgetHeight, undo=True, 
                        autoseparators=True, maxundo=-1)
    edCoord3d.grid(row=irow, rowspan=4, column=0, pady=0, ipady=0)
    irow += widgetHeight
    ...
    # widget command callback function
    def btCoord3d_clicked():
        ...
        return
    # set command callkback function
    ...
    btCoordImg.grid(row=irow, column=0)
    irow += 1
    #   edit text with text height
    widgetHeight = 2
    edCoordImg = tk.Text(win, width=40, height=widgetHeight, 
                         undo=True, autoseparators=True, maxundo=-1)
#    edCoordImg = tk.scrolledtext.ScrolledText(win, width=40, height=3, undo=True, autoseparators=True, maxundo=-1)
    edCoordImg.grid(row=irow, rowspan=3, column=0)
    irow += widgetHeight
    #   set initial text for demonstration
    ...
    # #######################################################
    # Button of [Generate chessb 3D ...]
    # #######################################################
    btChess3d = tk.Button(win, text='Generate chessboard 3D coord.')
    btChess3d.grid(row=9, column=0)
#    edChess3d = tk.Text(win, width=40, height=1, undo=True, autoseparators=True, maxundo=-1)
    edChess3d = tk.Text(win, width=40, height=1, undo=True, autoseparators=True, maxundo=-1)
    edChess3d.grid(row=10, rowspan=1, column=0)
    #   set initial text for demonstration
    edChess3d.delete(0., tk.END)
    edChess3d.insert(0., 'nCornersX nCornersY cellDx cellDy')

    def btChess3d_clicked():
        chessInfo = npFromString(edChess3d.get(0., tk.END))
        
        pass
    btChess3d.configure(command=btChess3d_clicked)
    ...
    # Calibration image
    #   file path text
    edFile = tk.Text(win, width=40, height=1)
#    edFile = tk.scrolledtext.ScrolledText(win, width=40, height=1, undo=True, autoseparators=True, maxundo=-1)
    edFile.grid(row=14, column=0)
    #   set initial text for demonstration
    try:
...
#        ufileDirFiles = uigetfiles(initialDirectory=initDir)
    ...
    edImgSize = tk.Text(win, width=40, height=1)
    edImgSize.grid(row=16, column=0)
    #   set initial text for demonstration
    try:
    ...
    btImgSize.grid(row=15, column=0)
...
#    edCmatGuess = tk.scrolledtext.ScrolledText(win, width=40, height=2, undo=True, autoseparators=True, maxundo=-1)
    edCmatGuess.grid(row=18, rowspan=2, column=0)
    #   set initial text for demonstration
    try:
    ...
    btCmatGuess.grid(row=17, column=0)
...
#    edDvecGuess = tk.scrolledtext.ScrolledText(win, width=40, height=2, undo=True, autoseparators=True, maxundo=-1)
    edDvecGuess.grid(row=21, rowspan=2, column=0)
    #   set initial text for demonstration
    try:
    ...
    btDvecGuess.grid(row=20, column=0)
...
#        cv.imshow("Points", imgr); cv.waitKey(0); 
    ...
    def btDrawPointsUndistort_clicked():
        ...
        try:
            ...
            tkCalib_printMessage('# No image point. Skipping plotting image points.')
            return
        # get projected points and draw points on undistorted image
        try:
            strPrjPoints= edCoordPrj.get(0., tk.END)
            prjPointsAll = npFromString(strPrjPoints).reshape((-1, 2))
            if prjPointsAll.shape[0] <= 0:
            ...
            thickness=4
            lineType=8
            imgud = drawPoints(imgud, prjPointsuAll, color=color, markerType=markerType, 
                              markerSize=markerSize,
                              thickness=thickness, lineType=lineType, savefile='.')
        except:
            tkCalib_printMessage('# No projected points. Skipping plotting them.')
        ...
        try:
            ...
            gridZs = npFromString(edGridZs.get()).reshape((-1)).astype(np.float64)
            if gridXs.size > 0 and gridYs.size > 0 and gridZs.size > 0:
                imgud = draw3dMesh(img=imgud, cmat=cmat, dvec=np.zeros((1,4), dtype=float),
                                   rvec=rvec, tvec=tvec,
                                   meshx=gridXs, meshy=gridYs, meshz=gridZs, 
                                   color=(64,255,255), thickness=2, shift=0, 
                                   savefile='.')
            ...
            print("# Skipping plotting grid.")
        # draw undistorted image points on undistorted image
        try:
            color=[0,255,0]; 
            markerType=cv.MARKER_CROSS
            markerSize=40
...
#        cv.imshow("Points on undistorted Image", imgr); cv.waitKey(0); 
        imshow2("Points of undistorted Image", imgud, winmax=(1600, 800), interp=cv.INTER_LANCZOS4)
        try:
